import {View, Text, TouchableOpacity, Image, ScrollView, ImageBackground} from 'react-native';
import BackSvg from '../assets/svg/backSvg';
import {useDispatch, useSelector} from 'react-redux';
import {buyItem, subtractBalance} from '../redux/slices/purchasesSlice';
import {selectSelectedItem} from '../redux/slices/selectedItemSlice';

const ShopScreen = ({ navigation }) => {
  const dispatch = useDispatch();
  const selectedItemId = useSelector(selectSelectedItem);
  const purchasedItems = useSelector((state) => state.purchases.purchasedItems);
  const balance = useSelector((state) => state.purchases.balance);

  const handlePurchase = (itemId, price) => {
    // const itemId = 'game_background';
    // const price = 50;
    console.log('itemId: ', itemId)
    console.log('price: ', price)
    if (balance >= price) {
      dispatch(buyItem({ itemId, price }));
      dispatch(subtractBalance(price));
    } else {
      alert('Not enough crowns');
    }
  };

  console.log('purchasedItems: ', purchasedItems)

  return (
    <ScrollView style={{flex: 1, backgroundColor: '#000'}}>
      <ImageBackground
        source={
          selectedItemId === 2
            ? require('../assets/img/back1.png')
            : selectedItemId === 3
              ? require('../assets/img/back2.png')
              : null
        }
        style={styles.container}
      >
        <TouchableOpacity
          onPress={() => {
            navigation.goBack();
          }}
          style={{
            backgroundColor: '#FFB300',
            borderWidth: 6,
            borderColor: '#e5d273',
            padding: 10,
            borderRadius: 15,
            alignItems: 'center',
            position: 'absolute',
            zIndex: 1,
            left: 24,
            top: 80,
          }}>
          <BackSvg />
        </TouchableOpacity>

        {selectedItemId === 1 && (
        <>
          <View style={{position: 'absolute', top: 0, left: 20}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 50,
                transform: [{rotate: '35deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>

          <View style={{position: 'absolute', top: '30%', left: 20}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 80,
                transform: [{rotate: '-35deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>

          <View style={{position: 'absolute', top: 50, right: '50%'}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 55,
                transform: [{rotate: '15deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>

          <View style={{position: 'absolute', top: 50, right: 50}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 80,
                transform: [{rotate: '-35deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>

          <View style={{position: 'absolute', top: 200, left: 180}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 100,
                transform: [{rotate: '-35deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>

          <View style={{position: 'absolute', bottom: '10%', left: 20}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 80,
                transform: [{rotate: '35deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>

          <View style={{position: 'absolute', bottom: 150, right: 80}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 75,
                transform: [{rotate: '-25deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>

          <View style={{position: 'absolute', bottom: '0%', right: 50}}>
            <Image
              resizeMode="contain"
              source={require('../assets/img/Crown1.png')}
              style={{
                width: 75,
                transform: [{rotate: '-25deg'}],
              }}
            />
            <View
              style={{
                position: 'absolute',
                width: '100%',
                height: '100%',
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                borderRadius: 10,
              }}
            />
          </View>
        </>
        )}
        <Text style={styles.title}>Shop</Text>
        <View
          style={{
            flexDirection: 'row',
            backgroundColor: '#fff',
            alignItems: 'center',
            padding: 12,
            borderWidth: 6,
            borderColor: '#e5d273',
            borderRadius: 15,
            position: 'absolute',
            top: 80,
            right: 26,
          }}>
          <Text
            style={{fontFamily: 'Nunito-Bold', color: '#000', fontSize: 12}}>
            {balance}
          </Text>
          <Image
            resizeMode="contain"
            source={require('../assets/img/Crown1.png')}
            style={{
              width: 24,
              height: 24,
              borderRadius: 15,
              marginLeft: 16,
            }}
          />
        </View>

        <View
          style={{
            backgroundColor: '#FFB300',
            flexDirection: 'row',
            width: '90%',
            marginBottom: 16,
            borderWidth: 6,
            borderColor: '#e5d273',
            padding: 14,
            borderRadius: 15,
            alignItems: 'center',
          }}>
          <Image
            // resizeMode="contain"
            source={require('../assets/img/back3.png')}
            style={{
              width: 120,
              borderRadius: 15,
              height: 120,
              marginRight: 16,
            }}
          />
          <View style={{alignItems: 'center', width: '60%'}}>
            <Text
              style={{
                fontFamily: 'Nunito-Bold',
                color: '#000',
                fontSize: 16,
                width: '80%',
              }}>
              game background
            </Text>
            <View
              style={{
                flexDirection: 'row',
                marginTop: 12,
                justifyContent: 'center',
                alignItems: 'center',
              }}>
              <Text
                style={{
                  fontFamily: 'Nunito-Bold',
                  color: '#000',
                  fontSize: 14,
                  marginRight: 16,
                }}>
                Exchange for:
              </Text>
              <View
                onPress={() => navigation.navigate('Onboarding2')}
                style={{
                  flexDirection: 'row',
                  backgroundColor: '#fff',
                  alignItems: 'center',
                  padding: 12,
                  borderWidth: 6,
                  borderColor: '#e5d273',
                  borderRadius: 15,
                }}>
                <Text
                  style={{
                    fontFamily: 'Nunito-Bold',
                    color: '#000',
                    fontSize: 12,
                  }}>
                  50
                </Text>
                <Image
                  resizeMode="contain"
                  source={require('../assets/img/Crown1.png')}
                  style={{
                    width: 24,
                    height: 24,
                    borderRadius: 15,
                    marginLeft: 16,
                  }}
                />
              </View>
            </View>

            <View
              // disabled={purchasedItems.some((item) => item.id === 1)}
              // onPress={() => {
              //   handlePurchase(1, 50);
              // }}
              style={[{
                width: '100%',
                alignItems: 'center',
                paddingVertical: 20,
                borderWidth: 6,
                borderColor: '#e5d273',
                borderRadius: 15,
                marginTop: 16,
              }, {backgroundColor: '#fff',}]}>
              <Text
                style={[{
                  fontFamily: 'Nunito-Bold',
                  fontSize: 14,
                }, {color: '#000'} ]}>
                Exchanged
              </Text>
            </View>
          </View>
        </View>

        <View
          style={{
            backgroundColor: '#FFB300',
            flexDirection: 'row',
            width: '90%',
            marginBottom: 16,
            borderWidth: 6,
            borderColor: '#e5d273',
            padding: 14,
            borderRadius: 15,
            alignItems: 'center',
          }}>
          <Image
            // resizeMode="contain"
            source={require('../assets/img/back1.png')}
            style={{
              width: 120,
              borderRadius: 15,
              height: 120,
              marginRight: 16,
            }}
          />
          <View style={{alignItems: 'center', width: '60%'}}>
            <Text
              style={{
                fontFamily: 'Nunito-Bold',
                color: '#000',
                fontSize: 16,
                width: '80%',
              }}>
              game background
            </Text>
            <View
              style={{
                flexDirection: 'row',
                marginTop: 12,
                justifyContent: 'center',
                alignItems: 'center',
              }}>
              <Text
                style={{
                  fontFamily: 'Nunito-Bold',
                  color: '#000',
                  fontSize: 14,
                  marginRight: 16,
                }}>
                Exchange for:
              </Text>
              <View
                style={{
                  flexDirection: 'row',
                  backgroundColor: '#fff',
                  alignItems: 'center',
                  padding: 12,
                  borderWidth: 6,
                  borderColor: '#e5d273',
                  borderRadius: 15,
                }}>
                <Text
                  style={{
                    fontFamily: 'Nunito-Bold',
                    color: '#000',
                    fontSize: 12,
                  }}>
                  100
                </Text>
                <Image
                  resizeMode="contain"
                  source={require('../assets/img/Crown1.png')}
                  style={{
                    width: 24,
                    height: 24,
                    borderRadius: 15,
                    marginLeft: 16,
                  }}
                />
              </View>
            </View>

            <TouchableOpacity
              disabled={purchasedItems.some((item) => item.id === 2)}
              onPress={() => {
                handlePurchase(2, 100);
              }}
              style={[{
                width: '100%',
                alignItems: 'center',
                paddingVertical: 20,
                borderWidth: 6,
                borderColor: '#e5d273',
                borderRadius: 15,
                marginTop: 16,
              }, purchasedItems.some((item) => item.id === 2) ? {backgroundColor: '#fff',} : {backgroundColor: '#000',}]}>
              <Text
                style={[{
                  fontFamily: 'Nunito-Bold',
                  fontSize: 14,
                }, purchasedItems.some((item) => item.id === 2) ? {color: '#000',} : {color: '#fff',} ]}>
                {purchasedItems.some((item) => item.id === 2) ? `Exchanged` : `Exchange`}
              </Text>
            </TouchableOpacity>
          </View>
        </View>

        <View
          style={{
            backgroundColor: '#FFB300',
            flexDirection: 'row',
            width: '90%',
            marginBottom: 16,
            borderWidth: 6,
            borderColor: '#e5d273',
            padding: 14,
            borderRadius: 15,
            alignItems: 'center',
          }}>
          <Image
            // resizeMode="contain"
            source={require('../assets/img/back2.png')}
            style={{
              width: 120,
              borderRadius: 15,
              height: 120,
              marginRight: 16,
            }}
          />
          <View style={{alignItems: 'center', width: '60%'}}>
            <Text
              style={{
                fontFamily: 'Nunito-Bold',
                color: '#000',
                fontSize: 16,
                width: '80%',
              }}>
              game background
            </Text>
            <View
              style={{
                flexDirection: 'row',
                marginTop: 12,
                justifyContent: 'center',
                alignItems: 'center',
              }}>
              <Text
                style={{
                  fontFamily: 'Nunito-Bold',
                  color: '#000',
                  fontSize: 14,
                  marginRight: 16,
                }}>
                Exchange for:
              </Text>
              <View
                style={{
                  flexDirection: 'row',
                  backgroundColor: '#fff',
                  alignItems: 'center',
                  padding: 12,
                  borderWidth: 6,
                  borderColor: '#e5d273',
                  borderRadius: 15,
                }}>
                <Text
                  style={{
                    fontFamily: 'Nunito-Bold',
                    color: '#000',
                    fontSize: 12,
                  }}>
                  150
                </Text>
                <Image
                  resizeMode="contain"
                  source={require('../assets/img/Crown1.png')}
                  style={{
                    width: 24,
                    height: 24,
                    borderRadius: 15,
                    marginLeft: 16,
                  }}
                />
              </View>
            </View>

            <TouchableOpacity
              disabled={purchasedItems.some((item) => item.id === 3)}
              onPress={() => {
                handlePurchase(3, 150);
              }}
              style={[{
                width: '100%',
                alignItems: 'center',
                paddingVertical: 20,
                borderWidth: 6,
                borderColor: '#e5d273',
                borderRadius: 15,
                marginTop: 16,
              }, purchasedItems.some((item) => item.id === 3) ? {backgroundColor: '#fff',} : {backgroundColor: '#000',}]}>
              <Text
                style={[{
                  fontFamily: 'Nunito-Bold',
                  fontSize: 14,
                }, purchasedItems.some((item) => item.id === 3) ? {color: '#000',} : {color: '#fff',} ]}>
                {purchasedItems.some((item) => item.id === 3) ? `Exchanged` : `Exchange`}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
      </ImageBackground>
    </ScrollView>
  );
};

const styles = {
  container: {
    backgroundColor: '#000',
    flex: 1,
    justifyContent: 'space-between',
    alignItems: 'center',
    paddingVertical: 100,
  },
  title: {
    fontFamily: 'Nunito-Bold',
    color: '#FFB300',
    fontSize: 42,
  },
  levelsContainer: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    justifyContent: 'center',
    gap: 16,
    width: '80%',
  },
  button: {
    backgroundColor: '#FFB300',
    borderWidth: 6,
    borderColor: '#e5d273',
    paddingHorizontal: 20,
    paddingVertical: 20,
    borderRadius: 15,
    alignItems: 'center',
    width: '30%',
    marginBottom: 16,
  },
  buttonText: {
    fontFamily: 'Nunito-Bold',
    color: '#000',
    fontSize: 24,
  },
  largeButton: {
    width: '40%',
    paddingVertical: 30,
  },
  largeText: {
    fontSize: 34,
  },
};

export default ShopScreen
